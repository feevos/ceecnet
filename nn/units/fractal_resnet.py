...
class FracTALResNet_unit(HybridBlock):
    def __init__(self, nfilters, ngroups=1, nheads=1, kernel_size=(3,3), dilation_rate=(1,1), norm_type = 'BatchNorm', norm_groups=None, ftdepth=5,**kwards):
        ...
        with self.name_scope():
            ...
            self.attn1 = FTAttention2D(nfilters, kernel_size=kernel_size, norm = norm_type, norm_groups = norm_groups,ftdepth=ftdepth)
            self.attn2 = FTAttention2D(nfilters, kernel_size=kernel_size, norm = norm_type, norm_groups = norm_groups,ftdepth=ftdepth)
